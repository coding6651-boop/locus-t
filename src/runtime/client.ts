...
import type { LLMProvider, LLMConfig, LLMResponse, Message, ToolDefinition } from '../providers/types.js'
import { checkHealth } from './health.js'

export class RuntimeClient implements LLMProvider {
  ...
  async chatStream(
    ...
    for await (const chunk of stream) {
      ...
      if (choice.finish_reason === 'tool_calls') {
        const tool_calls = Array.from(accs.values())
  ...
  async health(): Promise<{ ok: boolean; modelLoaded: boolean }> {
    const baseUrl = this.config.baseURL.replace(/\/v1$/, '')
    const result = await checkHealth(baseUrl)
    return { ok: result.healthy, modelLoaded: result.modelLoaded }
  }
}
